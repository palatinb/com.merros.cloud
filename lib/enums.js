...
function get_or_parse_namespace(namespace: Union[Namespace, str]) {
    if (isinstance(namespace, str)) {
        try {
            parsed_namespace = Namespace(namespace)
            return parsed_namespace
        } catch (ValueError)
        console.error("Namespace {namespace} is not currently handled/recognized."
        )
        return
    }
    else if (isinstance(namespace, Namespace)) {
        return namespace
    }
else {
        throw new ValueError("Unknown invalid namespace type. Only str/Namespace types are allowed here.")
    }
}
module.exports = Namespaces,OnlineStatus,LightMode,SprayMode,ThermostatV3Mode, get_or_parse_namespace()